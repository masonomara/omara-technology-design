...
![Three-source diagram](/content/projects/docket/images/6-three-source-diagram.png)
...
The first version (a Slack bot) proved that API calls and OAuth between Slack and the CRM worked, the conversation history was accessible, and the LLM could reference uploaded Org Context and the Knowledge Base. The architecture was validated, and I also discovered development pain points through this first build such as users being able to upload Org Context that I had to prioritize on the rebuild.

![Slack fundraising assistant](/content/projects/docket/images/5-slack-fundrasing-assistant.png)

## Why Lawyers

I needed an industry-specific CRM like Salesforce. I knew lawyers and administrative assistants that would be available to interview, and Clio was a growing CRM that users liked. I also explored what the tool would look like for legal clinics, so I set up separate logins for clinics and firms.
...
![Org context upload](/content/projects/docket/images/4-org-content-upload.png)
...
The second version's structure was too ambitious. I'm currently working on user feedback and opportunities to niche down as a legal clinic learning tool or focus on one practice type. The technical infrastructure has been validated: org context, Knowledge Base, and API calls combined. The scope needs to shrink.
